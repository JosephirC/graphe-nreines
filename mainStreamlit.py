...
count_all = st.checkbox("Compter toutes les solutions (plus lent)", value=False)
max_solutions = None if count_all else st.number_input("Nombre max de solutions à afficher", 1, 200, 10)
...
if "solutions" in st.session_state:
    ...
    st.success(f"✅ {total} solution(s) trouvée(s) en {st.session_state['time']:.3f} secondes")

    # Affichage du nombre total (avec message clair)
    if count_all:
        st.info(f"Nombre total exact de solutions pour N = {n} : **{total}**")
    else:
        st.caption(f"Seulement les {total} premières solutions sont affichées (pour aller plus vite).")
    ...
    for i in range(n):
        row = ""
        for j in range(n):
            cell = "♛" if board[i, j] == 1 else "⬜" if (i + j) % 2 == 0 else "⬛"
            row += cell + " "
